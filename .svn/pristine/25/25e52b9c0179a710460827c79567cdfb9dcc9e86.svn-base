<template>
  <div>
      ...
      <section class="examList relative" :style="{ '-webkit-overflow-scrolling': scrollMode,'height': wrapperHeight + 'px'}" ref="wrapper">
        ...
        <img src="../../assets/none.png" alt="" class="none" v-show = 'examList.length == 0'>

        <v-loadmore  :bottom-method="loadBottom" :bottom-all-loaded="allLoaded" :auto-fill="false" ref="loadmore" :bottomPullText='bottomText' :bottomLoadingText = 'bottomLoadingText' @bottom-status-change="handlebottomChange" :bottomDistance = '70' style="min-height:100%;" v-if = 'examList.length != 0'> 
